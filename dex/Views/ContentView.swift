...
struct ContentView: View {
                ...
                List {Section{
                    ForEach(pockemons) { pockemon in
                        NavigationLink (value: pockemon) {
                            HStack {
                                if pockemon.shinyImage == nil {
                                AsyncImage(url: pockemon.shinyURL) { image in
                                    image.resizable()
                                        .scaledToFit()
                                } placeholder: {
                                    ProgressView()
        ...
        Task {
            for i in 1...152 {
                do {
                    ...
                    pokemon.shinyURL = URL(string: newPok.sprites.frontShiny.isEmpty ? "" : newPok.sprites.frontShiny)
                    pokemon.spriteURL = URL(string: newPok.sprites.backShiny.isEmpty ? "" : newPok.sprites.backShiny)
                    if newPok.id % 2 == 0 {
    ...
    func saveImagesBinary() {
        Task {
            do {
                for pokemon in allPockemons {
